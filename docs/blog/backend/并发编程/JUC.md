...
!!!tip  
	原文仓库地址: [JavaNote](https://github.com/Seazean/JavaNote)
...
参考视频：[黑马 juc](https://www.bilibili.com/video/BV16J411h7Rd?vd_source=dff8e8da3e782503dba2b80a888e026c&p=143)
...
- sleep、wait、join 方法都会让线程进入阻塞状态，打断线程**通过抛出异常的方式表示被打断，但是会清空打断状态**（false）
...
读写锁用的是同一个 Sync 同步器，因此等待队列、state 等也是同一个，原理与 ReentrantLock 加锁相比没有特殊之处，不同是**写锁状态占了 state 的低 16 位，而读锁使用的是 state 的高 16 位**
